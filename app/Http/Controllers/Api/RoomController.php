...
class RoomController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if($withCondition) {
            $model = Ruangan::where(new Expression('lower('.$request->get('search').')'), 'like', '%'.strtolower($request->get('value')).'%')->orderBy($request->get('search'), 'asc')->get();
        } else {
            $model = Ruangan::orderBy('nama_ruangan', 'asc')->get();
        }
