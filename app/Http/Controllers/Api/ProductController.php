...
class ProductController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if($withCondition) {
            $model = Produk::where(new Expression('lower('.$request->get('search').')'), 'like', '%'.strtolower($request->get('value')).'%')->orderBy($request->get('search'), 'asc')->get();
        } else {
            $model = Produk::orderBy('namaproduk', 'asc')->get();
        }
